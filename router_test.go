...
func TestRouter_ServeHTTP_StaticRoutes(t *testing.T) {
	...
	for path, meth := range paths {
		r.Map(Methods{meth}, path, rec.Handler(path))
	}
	...
	testRouter_ServeHTTP(t, r, rec, tt)
}
...
func TestRouter_ServeHTTP_FallbackToParamRoute(t *testing.T) {
	t.Run("scenario 1", func(t *testing.T) {
		...
		testRouter_ServeHTTP_2(t, r, rec, tt)
	})
...
func BenchmarkRouter_ServeHTTP_MixedRoutes_X(b *testing.B) {
	r := New()
	...
	for path, meth := range paths {
		r.Map(Methods{meth}, path, fakeHandler())
	}
...
func BenchmarkRouter_ServeHTTP_ParamRoutes_RandomMethods(b *testing.B) {
	...
	for route, method := range routes {
		router.Map(Methods{method}, route, fakeHandler())
	}
...
func (r *recorder) Handler(path string) Handler {
