...
// releaseParamsHandler releases the request handler's internalParams object into the sync.Pool after execution.
// Here the downside is, if panics are not recovered in the chain, this wrapper won't get executed.
// Therefore, internalParams won't get released to the pool.
func releaseParamsHandler(pool *sync.Pool, handler HandlerFunc) HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request, route Route) (err error) {
		...
		route.Params.internal.reset()
		pool.Put(route.Params.internal)
		route.Params.internal = nil
		return
	}
}
